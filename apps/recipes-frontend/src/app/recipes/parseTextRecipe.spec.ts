...
describe('parseIngredientLine', () => {
  it('parsar mängd, enhet och namn', () => {
    expect(parseIngredientLine('• 2 dl grädde')).toEqual({ quantity: '2', unit: 'dl', name: 'grädde' });
  });

  it('parsar mängd och namn utan enhet', () => {
    expect(parseIngredientLine('• 2 avokado')).toEqual({ quantity: '2', unit: '', name: 'avokado' });
  });

  it('parsar ingrediens utan mängd', () => {
    expect(parseIngredientLine('• färsk persilja')).toEqual({ quantity: '', unit: '', name: 'färsk persilja' });
  });

  it('hanterar bråktal', () => {
    expect(parseIngredientLine('• 1/2 msk kebabkrydda')).toEqual({ quantity: '1/2', unit: 'msk', name: 'kebabkrydda' });
  });

  it('hanterar parenteser i namn', () => {
    expect(parseIngredientLine('• 100 g fetaost (ca)')).toEqual({ quantity: '100', unit: 'g', name: 'fetaost (ca)' });
  });
});
...
describe('parseTextRecipe', () => {
  ...
  it('extraherar ingredienser', () => {
    ...
    expect(result.ingredients?.[0]).toEqual({ quantity: '1', unit: '', name: 'gul lök' });
    expect(result.ingredients?.[3]).toEqual({ quantity: '2', unit: 'dl', name: 'grädde' });
    expect(result.ingredients?.[4]).toEqual({ quantity: '', unit: '', name: 'färsk persilja' });
  });
